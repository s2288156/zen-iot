...
public class PageResult<T> {
    ...
    public static <T> PageResult<T> of(Page<T> page) {
        return new PageResult<>(
                page.getContent(), page.getTotalElements(), page.getNumber() + 1, page.getSize(), page.getTotalPages());
    }

    /** 内容已映射为 DTO 时使用:mappedList 须与 page 当前页内容一一对应。 */
    public static <S, T> PageResult<T> of(Page<S> page, List<T> mappedList) {
        return new PageResult<>(
                mappedList, page.getTotalElements(), page.getNumber() + 1, page.getSize(), page.getTotalPages());
    }
