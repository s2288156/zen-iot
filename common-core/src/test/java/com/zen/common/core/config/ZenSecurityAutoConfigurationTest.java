...
class ZenSecurityAutoConfigurationTest {
      ...
      new WebApplicationContextRunner()
          .withConfiguration(
              AutoConfigurations.of(
                  ZenJwtAutoConfiguration.class, ZenSecurityAutoConfiguration.class));

  @Test
  void jwtSourceIsTheDefaultAndNeedsASecret() {
    runner
        .withPropertyValues("zen.jwt.secret=" + SECRET)
        .run(
            context -> {
              ...
              assertThat(context.getBean(UserContextResolver.class))
                  .isInstanceOf(JwtUserContextResolver.class);
            });
  }
  ...
  void gatewayHeaderSourceStartsWithoutAnySecret() {
    runner
        .withPropertyValues("zen.security.context-source=gateway-header")
        .run(
            context -> {
              assertThat(context).hasNotFailed();
              assertThat(context.getBean(UserContextResolver.class))
                  .isInstanceOf(GatewayHeaderUserContextResolver.class);
            });
  }
  ...
  void disablingSecurityRemovesTheInterceptor() {
    runner
        .withPropertyValues("zen.security.enabled=false")
        .run(
            context -> {
              assertThat(context).hasNotFailed();
    ...
    runner
        ...
        .run(
            context -> {
              ...
              assertThat(context.getBean(TokenRevocationChecker.class))
                  .isInstanceOf(RecordingChecker.class);
            });
  }
